-- Запрос на основные обращения в поликлинике (Formatted to String.format [percent char doubled])
SELECT
...
IF(LEFT(SERVICE.code,1) = 'T', 'T', MEDICAL_KIND.code) AS 'type',
-- пациент
...
INNER JOIN rbMedicalAidUnit AID_UNIT ON AID_UNIT.id = IF(MEDICAL_KIND.code = 'P' AND MKU.rbMedicalAidUnit_id = 2 AND RSLT.code <> 304
                                                           AND (getPreviousResult(PATIENT.id, ACT.endDate, E.contract_id, SERVICE.infis, E_TYPE.id, '%s') <> 'P304'), 1,  MKU.rbMedicalAidUnit_id)
-- Способ оплаты
...
-- исключение части случаев, как не требующих выгрузки в этот интервал (WMIS-22)
AND NOT (RSLT.code = 304 AND MEDICAL_KIND.code = 'P' AND AID_UNIT.code = '2')
-- исключение части случаев, как не требующих выгрузки в этот интервал (WMIS-40)
AND NOT (
    IDSP.code IN ('11', '94', '95', '96', '98')
    AND (
          (
            SUBSTRING(SERVICE.infis, 3, 3) NOT IN ('026', '031', '076', '081')
            AND MEDICAL_KIND.code <> 'P'
          ) OR (
            RSLT.regionalCode <> '2'
            AND MEDICAL_KIND.code = 'P'
          )
