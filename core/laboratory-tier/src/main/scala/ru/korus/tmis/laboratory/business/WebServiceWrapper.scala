...
class WebServiceWrapper[Port: Manifest](wsdl: URL, serviceAddr: QName) {
  ...
  def serviceUrl_=(v: URL) = bp foreach {
    it => it.getRequestContext.put(ENDPOINT_ADDRESS_PROPERTY, v.toString)
  }

  def username_=(v: String) = bp foreach {
    it => it.getRequestContext.put(USERNAME_PROPERTY, v)
  }

  def password_=(v: String) = bp foreach {
    it => it.getRequestContext.put(PASSWORD_PROPERTY, v)
  }
