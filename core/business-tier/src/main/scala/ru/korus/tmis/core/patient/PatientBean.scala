...
class PatientBean
  ...
  def getCurrentPatientsForDepartment(userData: AuthData) = {
    ...
    val id = userData.user.getOrgStructure ?!! {
      _.getId.intValue
    } getOrElse {
  ...
  def getAllPatients(requestData: PatientRequestData) = {
    ...
    val page = requestData.page match {
      ...
      case _ => {
        requestData.page.toInt - 1
      } //c клиента приходит номер страницы начиная с 1
    }
    ...
    val sortField = requestData.sortingField match {
      ...
      case "patientCode" => {
        "id"
      } //TODO: в бд нет поля "код пациента"
      case _ => {
  ...
  def savePatient(patientEntry: PatientEntry, userData: AuthData): PatientEntry = {
    ...
    try {
      ...
      serverIdCards.foreach(
        (serverIdCard) => {
          val result = clientIdCards.find {
            element => element.getId() == serverIdCard.getId().intValue()
          }
      ...
      serverPolicies.foreach(
        (serverPolicy) => {
          val result = clientPolicies.find {
            element => element.getId() == serverPolicy.getId().intValue()
          }
      ...
      var result = serverAddresses.find {
        element => element.getAddressType == 0
      }
      var registeredAddres = result.getOrElse(null)
      result = serverAddresses.find {
        element => element.getAddressType == 1
      }
      ...
      serverAllergies.foreach(
        (serverAllergy) => {
          val result = clientAllergies.find {
            element => element.getId() == serverAllergy.getId().intValue()
          }
      ...
      patientIntolerances.foreach(
        (serverIntolerance) => {
          val result = clientDrugIntolerances.find {
            element => element.getId() == serverIntolerance.getId().intValue()
          }
      ...
      serverRelations.foreach(
        (serverRelation) => {
          val result = clientRelations.find {
            element => element.getId() == serverRelation.getId().intValue()
          }
      ...
      serverSocScatuses.foreach(serverStatus => {
        serverStatus.getSocStatusClass.getId.intValue() match {
          case 32 => {
            //Инвалидности
            val result = clientDisabilities.find {
              element => element.getId() == serverStatus.getId().intValue()
            }
            ...
            if (clientDisability != null) {
              ...
              if (socStatusTypeId > 0) {
                ...
                tempServerStatus = dbSocStatus.insertOrUpdateClientSocStatus(
                  ...
                  32,
                  socStatusTypeId,
                  if (clientDisability.getDocument() != null) {
                    clientDisability.getDocument().getId()
                  } else {
                    0
                  },
                  ...
                  patient,
                  if (clientDisability.getBenefitsCategory() != null) {
                    clientDisability.getBenefitsCategory().getId()
                  } else {
                    0
                  },
          ...
          case 34 => {
            //Занятость
            val result = clientOccupations.find {
              element => element.getId() == serverStatus.getId().intValue()
            }
            ...
            if (clientOccupation != null) {
              ...
              serverWorks.foreach(serverWork => {
                //занятости
                val result = clientWorks.find {
                  element => element.getId() == serverWork.getId().intValue()
                }
                ...
                if (clientWork != null) {
                  ...
                  tempServerWork = dbClientWorks.insertOrUpdateClientWork(
                    ...
                    if (clientWork.getRank() != null) {
                      clientWork.getRank().getId()
                    } else {
                      0
                    },
                    if (clientWork.getForceBranch() != null) {
                      clientWork.getForceBranch().getId()
                    } else {
                      0
                    },
                    usver
