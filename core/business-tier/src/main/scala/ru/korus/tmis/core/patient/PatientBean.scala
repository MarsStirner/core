...
class PatientBean
  ...
  def savePatient(id: Int, patientEntry: PatientEntry, userData: Staff): PatientEntry = {
    ...
    val omsPolicies = clientPolicies.filter(p => {
      ...
      if (p.getPolicyType.getId == null || p.getPolicyType.getId.equals(0)) false
      else {
    ...
    omsPolicies.foreach(p => {
      if (p.getRangePolicyDate.getStart != null && p.getRangePolicyDate.getEnd != null) {
        if (omsPolicies.exists(o => {
          val startDate = o.getRangePolicyDate.getStart
    ...
    serverSocScatuses.filter(sw => {
      ...
      serverWorks.foreach(serverWork => {
        if (serverWork != null && serverWork.getId != null) {
          dbClientWorks.deleteClientWork(serverWork.getId.intValue(), userData)
        }
      })
    })
    ...
    if (bloodType > 0 && (
      bh == null ||
        bh.size() <= 0 ||
        bh != null && bh.size() > 0 && bh.get(0).getBloodType.getId.intValue() != bloodType)) {
      ...
      //Проверка дат (форматы хранения разные :()
  ...
  def insertBloodTypeForPatient(id: Int, data: BloodHistoryData, staff: Staff) = {
    ...
    //Создаем запись в BloodHistory
    val bloodhistory = dbBloodHistoryBean.createBloodHistoryRecord(id, bloodTypeId, bloodDate, staff)
    dbManager.persist(bloodhistory)
