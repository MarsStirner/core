...
public class AppealTestCase {
    ...
    @EJB
    private static AuthStorageBeanLocal authStorage;
    ...
    private static String WEB_AUTH_NAME = "tmis-ws-medipad/rest/tms-auth";
    private static String BASE_URI = "http://localhost:" + 8181;
    ...
    public static void setUp() throws Exception {
        ...
        //Авторизация
        AppealTestCase.testAuth();
    }

    private static void testAuth() throws Exception { //Проверка авторизации - создание тестового токена для остальных тестов
        try {

            AuthData authData = AppealTestCase.authStorage.createToken("ДБалашов", "c4ca4238a0b923820dcc509a6f75849b", 24);
            assertTrue("testAppeal>testAuth: >>> AuthToken is :" , false);
            assertTrue("testAppeal>testAuth: >>> AuthToken is not exists!",(authData.getAuthToken().toString()!=null && !authData.getAuthToken().toString().isEmpty()));
            AppealTestCase.authData = authData;
            assertTrue("testAppeal>testAuth: >>> AuthToken is :" + AppealTestCase.authData.getAuthToken().getId(), false);
            /*String REST_AUTH_URI = BASE_URI + "/" + WEB_AUTH_NAME  + "/" + "auth";
            WebResource resource  = Client.create().resource(REST_AUTH_URI);
            MultivaluedMap<String, String> queryParams = new MultivaluedMapImpl();
            queryParams.add("login", "ДБалашов");
            queryParams.add("passwd", "c4ca4238a0b923820dcc509a6f75849b");
            queryParams.add("role", "24");
            ClientResponse response = resource.queryParams(queryParams).get(ClientResponse.class);   // GET

            String crEntityStr = response.getEntity(String.class);
            ClientResponse.Status crStatus = response.getClientResponseStatus();
            //assertTrue("testAppeal>testAuth: >>>ClientResponse.Status != OK", ClientResponse.Status.OK == crStatus);
            assertTrue("testAppeal>testAuth: >>>Person was not founded! - %s".format(crEntityStr), crEntityStr.contains("Балашов"));

            JAXBContext jaxb = JAXBContext.newInstance(AuthData.class);
            JSONObject json = new JSONObject("{\"authData\":" + crEntityStr + "}");
            Configuration config = new Configuration();
            MappedNamespaceConvention convention = new MappedNamespaceConvention(config);
            MappedXMLStreamReader xmlReader = new MappedXMLStreamReader(json, convention);

            Unmarshaller um = jaxb.createUnmarshaller();
            AuthData authData = (AuthData)um.unmarshal(xmlReader);
            assertTrue("testAppeal>testAuth: >>> AuthToken is not exists!",(authData.getAuthToken().toString()!=null && !authData.getAuthToken().toString().isEmpty()));
            assertTrue("testAppeal>testAuth: >>>Service return error message! - %s".format(crEntityStr), !crEntityStr.contains("\"errorMessage\":"));
            ...
            AppealTestCase.authData = authData;
                ...
                e.printStackTrace();
                assertTrue("testAppeal>testAuth: >>>UnmarshalException: %s".format(e.getMessage()), false); */
        }
        catch (Exception e) {
                e.printStackTrace();
                assertTrue("testAppeal>testAuth: >>>Exception: %s".format(e.getMessage()), false);
            }
        }
