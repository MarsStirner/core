...
trait Configuration extends Reflective {
  private lazy val conversions = Map[Manifest[_], String => Any](
    ...
    manifest[SimpleDateFormat] -> {
      new SimpleDateFormat(_)
    },
    manifest[Set[_]] -> {
      it: String => it.split(",").toSet[String]
    }
  )
  ...
  lazy val children =
    this.
      methods.
      filter {
      case (_, field) => classOf[Configuration].isAssignableFrom(field.getReturnType)
    }.
      collect {
      case (name, field) => (name, field.invoke(this).asInstanceOf[Configuration])
    }
  ...
  lazy val fields =
    this.
      ...
      collect {
      case (name, field) => (name, field)
    }

  def setSetting(path: String, value: String): Boolean = {
    def catchy[A](v: => A): Option[A] = try Some(v) catch {
      case ex => None
    }
    ...
    tokens match {
      ...
      case Array(name) => fields.
        ...
        flatMap {
        it => convert(manifestFromClass(it.getReturnType))
      }. // take the
        // conversion function
        flatMap {
        func => catchy {
