...
class TalonSPOEntry {
  ...
  def this(event: Event) {
    ...
    if (event != null) {
      ...
      this.typeOfPayment = new IdNameContainer(event.getEventType.getFinanceId.intValue(), "") //достать rbFinance.name
...
class TalonSPODataList {
  ...
  def this(mutableEvents: java.util.Map[Event, java.util.Map[Object, Object]],
    ...
    mutableEvents.foreach(f => this.data += new TalonSPOListEntry(f._1,
      f._2))
  }
}
