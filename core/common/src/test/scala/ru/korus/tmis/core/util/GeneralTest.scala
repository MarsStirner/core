...
import ru.korus.tmis.util.{Types, General}

import org.junit.Assert._
import Types.{JList, JLinked, JDouble, JInteger}
...
class GeneralTest {
  ...
  def test_nullity {
    ...
    assert(vi2 ?! { _.head } ?!! { _.byteValue } == None)
    ...
    assert(testClass ?! {_.next} ?! {_.getNext} ?! {_.next} ?! {_.next} ?! {_.next} == null)
    assert(testClass ?! {_.next} ?! {_.getNext} ?! {_.next} == null)
    assert(testClass ?! {_.next} ?! {_.getNext} == TestClass(null))

    assert(testNull ?! {_.next} ?! {_.getNext} ?! {_.next} ?! {_.next} ?! {_.next} == null)
    assert(testNull ?! {_.next} ?! {_.getNext} ?! {_.next} == null)
    assert(testNull ?! {_.next} ?! {_.getNext} == null)
...

    {
      ...
      assert((a ?! {_.b} ?! {_.c} ?!! {_.d} getOrElse 0) == 2)
    }
...

    {
      ...
      assert((a ?! {_.b} ?! {_.c} ?!! {_.d} getOrElse 0) == 0)
    }
...

    {
      val nn: JInteger = 23
      assert(nn ?: 25 == 23)
      lazy val rvalue: JInteger = {throw new NullPointerException}
      assert(nn ?: rvalue == 23)
      def rvalue2 : JInteger = {throw new NullPointerException}
  ...
  def test_flow_implicits {
    ...
    var vi = new JLinked[Int]() stating { it => it.add(2); it.add(3) } stating { _.add(1) }
