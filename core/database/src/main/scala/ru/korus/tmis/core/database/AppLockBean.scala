...
import org.eclipse.persistence.queries.{StoredProcedureCall, ValueReadQuery}
import ru.korus.tmis.core.auth.AuthData
import ru.korus.tmis.core.entity.model.{Action, AppLock, AppLockDetail}
...
class AppLockBean
  ...
  def getAppLock(table: String,
    ...
    val args = new util.LinkedList[AnyRef]()
    args.add(table)
    ...
    result.size match {
      case 0 => new AppLockStatus(0, AppLockStatusType.busy)
      case size =>
        val data = result.split(' ')
        val lockStatus = data(0)
        val lockId = data(1).toInt
        if (lockStatus == "0") {
          new AppLockStatus(lockId, AppLockStatusType.alreadyLocked)
        } else {
          new AppLockStatus(lockId, AppLockStatusType.lock)
        }
    }
  }
