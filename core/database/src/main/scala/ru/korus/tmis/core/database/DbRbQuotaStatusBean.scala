...
class DbRbQuotaStatusBean
  ...
  def getRbPolicyTypeById(id: Int): RbQuotaStatus = {
  ...
  def getCountOfRbQuotaStatusWithFilter(filter: Object) = {
    ...
    var typed = em.createQuery(AllRbQuotaStatusWithFilterQuery.format(
      "count(r)",
      queryStr.query,
      ""),
      classOf[Long])
  ...
  def getAllRbQuotaStatusWithFilter(page: Int, limit: Int, sortingField: String, sortingMethod: String, filter: Object): java.util.LinkedList[Object] = {
    var queryStr: QueryDataStructure = if (filter.isInstanceOf[DictionaryListRequestDataFilter]) {
      filter.asInstanceOf[DictionaryListRequestDataFilter].toQueryStructure()
    } else {
      new QueryDataStructure()
