...
public class PharmacyBean implements PharmacyBeanLocal {
    ...
    private static final int LAST_ACTIONS = 300;
    ...
    public Request createRequest(final Action action, PharmacyStatus status, final ToLog toLog)
        ...
        if(PharmacyStatus.RESEND == status) {
            final OrgStructure structure = FlatCode.LEAVED.getCode().equalsIgnoreCase(actionType.getFlatCode()) ||
                    FlatCode.DEL_RECEIVED.getCode().equalsIgnoreCase(actionType.getFlatCode()) ? null : getReceivedOrgStructure(action);
            toLog.addN("receive orgStructure [#], [#]", structure.getId(), structure == null ? "N/A" : structure.getName());
